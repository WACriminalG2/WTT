...
let magicItems : Ware[] = [
    new Ware("Amulet of Oona the Unburnt",292).autoprice('Uncommon','Major','Charged').rarity('Uncommon').desc("This amulet hangs from a simple gold chain, and     features a black stone at the center. While wearing the amulet, if you take fire damage, you can use your reaction to absorb any heat that would harm you, absorbing them into the amulet. The amulet can absorb a maximum of 6d6 fire damage on behalf of the wearer. When the amulet is discovered, roll the dice to determine the amulet’s maximum capacity.").desc("When the amulet’s absorption capacity maximum is reached, the amulet provides the wearer with resistance to the remainder of the fire damage taken, and any further fire damage they take.").desc("The amulet remains pleasantly warm, slowly discharging any heat it has absorbed. Each day at dawn, roll 6d6 for the amount of absorbed fire damage that has been harmlessly discharged from the amulet’s maximum absorption capacity.").mic('Wondrous Item').classify('MOJ'),

    new Ware("Apprentice's Dueling Robe",292).autoprice('Uncommon','Major','Permanent').rarity('Uncommon').attune('a sorcerer, warlock, or wizard').mic('Wondrous Item').desc("This set of periwinkle robes is made from a high-quality wool bearing a large, embroidered patch in the shape of a shield over the left breast. When you cast a 1st-level spell or higher while wearing the robes, you gain a +1 bonus to your Armor Class and saving throws until the start of your next turn.").classify('MOO0'),

    new Ware("Avian Construct",293).autoprice('Rare','Minor','Charged').rarity('Rare').mic('Wondrous Item').desc("This item takes the form of a delicate, aerodynamically sound mechanical bird or dragon. An inconspicuous button under its left wing activates its reactor. After a few seconds, it becomes active and aware, and imprints upon the first individual it senses. For the duration, it obeys their commands.").desc("Deduct the time the avian construct is active, in increments of 1 minute, from its reactor’s maximum operation duration of 2 hours. Pressing the button underneath the left wing again deactivates it. The Avian Construct is unable to deactivate itself. For each uninterrupted period of 12 hours the reactor remains inactive, it regains 1 hour of operation. If the reactor is completely depleted, the avian construct loses its imprint. If not, it remains imprinted on the same individual the next time it is activated.").desc("While inactive, the avian construct is unconscious, restrained and incapacitated. While active, the avian construct shares statistics of a hawk, except that it is a construct instead of a beast, and it understands Common, but cannot speak, and it has one additional bonus action: ").desc(new AbilityText("Record/Playback","The avian construct records up to 1 minute of what it sees and hears. It can play that recording back as three-dimensional illusion if the individual it is imprinted upon presses an inconspicuous button hidden underneath its right wing. The recorded illusion is played back at a scale of 1:10. These recordings are immediately erased if its reactor is depleted.")).desc("If the avian construct is reduced to 0 hit points, it can be repaired with 50g of materials, 1 hour of labor and a successful DC 15 check made with tinker’s tools.").classify("MCN"),

    new Ware("Balthazaar's Big Boring Book",293).mic('Wondrous Item').rarity('Very Rare').autoprice('Very Rare',"Minor","Limited").desc("A leather-bound book with a marvelously detailed cover: two dragons locked in a battle, one cast in silver, the other in gold.").desc(new AbilityText("Lulling Allure","An unoccupied (not in combat or otherwise immediately preoccupied with a task) humanoid that can read and gazes directly at the cover must succeed a DC 14 Wisdom saving throw or be compelled to open the book and read it. The reader is at first completely engrossed by the book’s contents, but after 1 minute, they grow increasingly disaffected and bored. The reader must then succeed a DC 14 Wisdom saving throw at disadvantage or fall asleep for 1d8 hours. The book can only affect a creature once.")).classify('MB')
]
...
sb.cast(spellbook);
sb.stock(weapons).stock(poisons);
